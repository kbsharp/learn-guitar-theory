...
<svelte:head>
	...
	<meta
		name="description"
		content="Five interactive music theory tools for guitarists. No sheet music required."
	/>
</svelte:head>
	...
	<div class="fretboard-bg" aria-hidden="true">
		...
		{#each notes as note}
			<div
				class="note"
				class:root={note.r}
				style="left: {noteLeft(note.f)}; top: {noteTop(
					note.s
				)}; animation-delay: {noteDelay(note.f)};"
			></div>
		{/each}
...
<style lang="scss">
	...
	.hero {
		...
		background:
			radial-gradient(
				...
				color-mix(in srgb, var(--accent-note) 4%, transparent) 0%,
				transparent 70%
			),
			var(--bg-base);
	}
	...
	.hero-content {
		...
		.h1-line {
			...
			&.bold {
				...
				color: var(--text-primary);
				transition:
					opacity 1s ease 0.38s,
					transform 1s ease 0.38s;
			}
		}
	...
	.feature-row {
		...
		&.accent-cyan::after {
			background: color-mix(in srgb, var(--accent-note) 3%, transparent);
		}
		&.accent-rose::after {
			background: color-mix(in srgb, var(--accent-tonic) 3%, transparent);
		}
		&.accent-mixed::after {
			background: linear-gradient(
				135deg,
				color-mix(in srgb, var(--accent-note) 3%, transparent),
				color-mix(in srgb, var(--accent-tonic) 3%, transparent)
			);
		}
		...
		&:hover {
			...
			&.accent-cyan .feature-arrow {
				color: var(--accent-note);
			}
			&.accent-rose .feature-arrow {
				color: var(--accent-tonic);
			}
			&.accent-mixed .feature-arrow {
				color: var(--accent-note);
			}

			&.accent-cyan .feature-num {
				color: var(--accent-note);
			}
			&.accent-rose .feature-num {
				color: var(--accent-tonic);
			}
			&.accent-mixed .feature-num {
				color: var(--accent-note);
			}
		}
